...
void dd_load_sprite( int nr )
{
  ...
  if ( ! optr )
  { // try to load from single png file
  ...
  if ( ! optr )
  { // try to load from pnglib
    fp = load_pnglib( nr );
    if ( fp )
      optr = dd_load_png( fp, &xs, &ys, &alpha, &alphacnt );
  }
  if ( ! optr )
  { // try to load from single bmp file
    sprintf( buf, "%sgfx\\%05d.bmp", path, nr );
    optr = dd_load_bitmap( buf, &xs, &ys, suro );
  }
  if ( ! optr )
  { // try to load from bmplib
...
unsigned short *dd_load_png( FILE *fp, int *xs, int *ys, unsigned char **alpha_ptr, int *alphacnt_ptr )
{
  ...
  for ( n = 0; n < y; n++ )
  {
    for ( m = 0; m < x; m++ )
    {
      ...
      if ( ( do_alpha == 0 && a < 64 ) || ( do_alpha == 1 && a < 192 ) || ( do_alpha == 2 && a < 255 ) )
