...
void plr_change(int nr)
{
  ...
  if (ch[cn].attack_cn != cpl->attack_cn || ch[cn].goto_x != cpl->goto_x || ch[cn].goto_y != cpl->goto_y ||
      ch[cn].misc_action != cpl->misc_action || ch[cn].misc_target1 != cpl->misc_target1 || ch[cn].misc_target2 != cpl->misc_target2)
  {
    buf[0]         = SV_SETTARGET;
...
void plr_getmap_complete(int nr)
{
  ...
  for (n = YSCUT * TILEX + XSCUT, m = xs + ys * MAPX, y = ys; y < ye; y++, m += MAPX - TILEX + XSCUT + XECUT, n += XSCUT + XECUT)
  {
    for (x = xs; x < xe; x++, n++, m++)
    {
      ...
      if (map[m].flags & (MF_GFX_INJURED | MF_GFX_INJURED1 | MF_GFX_INJURED2 | MF_GFX_DEATH | MF_GFX_TOMB | MF_GFX_EMAGIC | MF_GFX_GMAGIC |
                          MF_GFX_CMAGIC | MF_UWATER))
      {
      ...
      if (see[cn].vis[tmp + 0 + 0] || see[cn].vis[tmp + 0 + 40] || see[cn].vis[tmp + 0 - 40] || see[cn].vis[tmp + 1 + 0] ||
          see[cn].vis[tmp + 1 + 40] || see[cn].vis[tmp + 1 - 40] || see[cn].vis[tmp - 1 + 0] || see[cn].vis[tmp - 1 + 40] ||
          see[cn].vis[tmp - 1 - 40])
        visible = 1;
      else
        visible = 0;
...
void plr_getmap_fast(int nr)
{
  ...
  for (n = YSCUTF * TILEX + XSCUTF, m = xs + ys * MAPX, y = ys; y < ye; y++, m += MAPX - TILEX + XSCUTF + XECUTF, n += XSCUTF + XECUTF)
  {
    for (x = xs; x < xe; x++, n++, m++)
    {
      ...
      if (map[m].flags &
          (MF_GFX_INJURED | MF_GFX_INJURED1 | MF_GFX_INJURED2 | MF_GFX_DEATH | MF_GFX_TOMB | MF_GFX_EMAGIC | MF_GFX_GMAGIC | MF_GFX_CMAGIC))
      {
        if (map[m].flags & MF_GFX_INJURED)
          smap[n].flags |= INJURED;
        if (map[m].flags & MF_GFX_INJURED1)
