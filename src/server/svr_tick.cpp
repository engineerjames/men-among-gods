...
{
  return _mcmp( static_cast< unsigned char* >( a ), static_cast< unsigned char* >( b ), len );
}
...
{
  return _fdiff( static_cast< unsigned char* >( a ), static_cast< unsigned char* >( b ), len );
}
...
{
  return _mcpy( static_cast< unsigned char* >( a ), static_cast< unsigned char* >( b ), len );
}
...
void plr_cmd_setuser( int nr )
{
  ...
  case 2:
    ...
    if ( pos != 65 )
      break;
    if ( strlen( ch[ cn ].text[ 0 ] ) > 3 && strlen( ch[ cn ].text[ 0 ] ) < 38 && ( ch[ cn ].flags & CF_NEWUSER ) )
    {
...
void plr_cmd( int nr )
{
  ...
  if ( player[ nr ].inbuf[ 0 ] != CL_CMD_AUTOLOOK && player[ nr ].inbuf[ 0 ] != CL_PERF_REPORT && player[ nr ].inbuf[ 0 ] != CL_CMD_CTICK )
...
void plr_logout( int cn, int nr, int reason )
{
  ...
  if ( cn > 0 && cn < MAXCHARS && ( ch[ cn ].player == nr || nr == 0 ) && ( ch[ cn ].flags & ( CF_PLAYER ) ) &&
       ! ( ch[ cn ].flags & CF_CCP ) )
  {
...
void plr_change_stat( int nr, unsigned char* a, unsigned char* b, unsigned char code, unsigned char n )
{
  ...
  if ( *( unsigned long* ) a != *( unsigned long* ) b || *( unsigned short* ) ( a + 4 ) != *( unsigned short* ) ( b + 4 ) )
  {
    buf[ 0 ] = code;
    buf[ 1 ] = n;
...
void plr_change( int nr )
{
  ...
  if ( cpl->points != ch[ cn ].points || cpl->points_tot != ch[ cn ].points_tot || cpl->kindred != ch[ cn ].kindred )
  {
  ...
  for ( n = 0; n < TILEX * TILEY; n++ )
  {
    ...
    {
      int bp = 0, l, bl = 0, pe;
      static int ( *lfunc[] )( int, int, struct cmap*, struct cmap* ) = { cl_light_one, cl_light_three, cl_light_seven, cl_light_26 };
  ...
  while ( 1 )
  {
    ...
    tmp = static_cast< unsigned char* >(
        fdiff( cmap + n, smap + n, sizeof( struct cmap ) * ( TILEX * TILEY - n ) ) ); // find address of first difference
    if ( ! tmp )
    ...
    if ( cmap[ n ].ch_speed != smap[ n ].ch_speed || cmap[ n ].ch_nr != smap[ n ].ch_nr || cmap[ n ].ch_id != smap[ n ].ch_id )
    {
      buf[ 1 ] |= 64;
  ...
  if ( ch[ cn ].attack_cn != cpl->attack_cn || ch[ cn ].goto_x != cpl->goto_x || ch[ cn ].goto_y != cpl->goto_y ||
       ch[ cn ].misc_action != cpl->misc_action || ch[ cn ].misc_target1 != cpl->misc_target1 ||
...
static int tmp_see(int frx,int fry,int tox,int toy)
{
        ...
        while (1) {
                ...
                if (panic++>25) {
                        xlog("f=%d,%d t=%d,%d (%d,%d)
d=%d,%d",(int)(fx>>16),(int)(fy>>16),(int)(tx>>16),(int)(ty>>16),(int)(x>>16),(int)(y>>16),(int)dx,(int)dy); return 1;
                }
        }
}
...
void plr_getmap_complete( int nr )
{
  ...
  for ( n = YSCUT * TILEX + XSCUT, m = xs + ys * MAPX, y = ys; y < ye; y++, m += MAPX - TILEX + XSCUT + XECUT, n += XSCUT + XECUT )
  {
    for ( x = xs; x < xe; x++, n++, m++ )
    {
      ...
      if ( do_all || map[ m ].it || map[ m ].ch || mcmp( &player[ nr ].xmap[ n ], &map[ m ], sizeof( struct map ) ) )
      {                                                                   // any change on map data?
      ...
      if ( map[ m ].flags & ( MF_GFX_INJURED | MF_GFX_INJURED1 | MF_GFX_INJURED2 | MF_GFX_DEATH | MF_GFX_TOMB | MF_GFX_EMAGIC |
                              MF_GFX_GMAGIC | MF_GFX_CMAGIC | MF_UWATER ) )
      {
      ...
      if ( see[ cn ].vis[ tmp + 0 + 0 ] || see[ cn ].vis[ tmp + 0 + 40 ] || see[ cn ].vis[ tmp + 0 - 40 ] || see[ cn ].vis[ tmp + 1 + 0 ] ||
           see[ cn ].vis[ tmp + 1 + 40 ] || see[ cn ].vis[ tmp + 1 - 40 ] || see[ cn ].vis[ tmp - 1 + 0 ] ||
           see[ cn ].vis[ tmp - 1 + 40 ] || see[ cn ].vis[ tmp - 1 - 40 ] )
        visible = 1;
      else
        visible = 0;
      ...
      else if ( ( co = map[ m ].it ) != 0 &&
                ( ! ( it[ co ].flags & ( IF_TAKE | IF_HIDDEN ) ) || ( visible && do_char_can_see_item( cn, co ) ) ) )
      {
        if ( it[ co ].active )
        {
...
int check_valid( int cn )
{
  ...
  for ( n = 0; n < 20; n++ )
  {
    ...
    if ( ( in = ch[ cn ].spell[ n ] ) != 0 )
    {
      if ( it[ in ].carried != cn || it[ in ].used != USE_ACTIVE )
      {
        xlog( "Reset spell item %d (%s,%d,%d) from char %d (%s)", in, it[ in ].name, it[ in ].carried, it[ in ].used, cn, ch[ cn ].name );
        ch[ cn ].spell[ n ] = 0;
      }
    }
  }
