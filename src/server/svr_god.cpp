...
{
  static char  buf[ 40 ];
  static char* syl1[] = { "thi",  "ar",  "an", "un",  "iss", "ish", "urs", "ur",  "ent", "esh", "ash", "jey", "jay", "dur", "lon", "lan",
                          "len",  "lun", "so", "lur", "gar", "cry", "au",  "dau", "dei", "zir", "zil", "sol", "luc", "ni",  "bus", "mid",
                          "err",  "doo", "do", "al",  "ea",  "jac", "ta",  "bi",  "vae", "rif", "tol", "nim", "ru",  "li",  "fro", "sam",
                          "beut", "bil", "ga", "nee", "ara", "rho", "dan", "va",  "lan", "cec", "cic", "cac", "cuc", "ix",  "vea", "cya",
                          "hie",  "bo",  "ni", "do",  "sar", "phe", "ho",  "cos", "sin", "tan", "mul", "har", "gur", "tar", "a",   "e",
                          "i",    "o",   "u",  "je",  "ho",  "if",  "jai", "coy", "ya",  "pa",  "pul", "pil", "rez", "rel", "rar", "dom",
                          "rom",  "tom", "ar", "ur",  "ir",  "er",  "yr",  "li",  "la",  "lu",  "lo" };
  static char* syl2[] = { "tar", "tur", "kar", "kur", "kan", "tan", "gar", "gur", "run" };
  static char* syl3[] = { "a", "e", "i", "o", "u" };
...
int god_create_char( int temp, int withitems )
{
  ...
  sprintf( ch[ n ].description, "%s is a %s%s%s%s%s. %s%s%s looks somewhat nondescript.", ch[ n ].name,
           ( ch[ n ].kindred & KIN_TEMPLAR ) ? "Templar" : "", ( ch[ n ].kindred & KIN_HARAKIM ) ? "Harakim" : "",
           ( ch[ n ].kindred & KIN_MERCENARY ) ? "Mercenary" : "", ( ch[ n ].kindred & KIN_SEYAN_DU ) ? "Seyan'Du" : "",
           ! ( ch[ n ].kindred & ( KIN_TEMPLAR | KIN_HARAKIM | KIN_MERCENARY | KIN_SEYAN_DU ) ) ? "Monster" : "",
           ( ch[ n ].kindred & KIN_MALE ) ? "He" : "", ( ch[ n ].kindred & KIN_FEMALE ) ? "She" : "",
...
void god_info( int cn, int co )
{
  ...
  if ( IS_PLAYER( co ) )
  {
    do_char_log( cn, 2, "%s %s%s%s Pts/need=%s/%s.\n", who_rank_name[ points2rank( pts ) ], ch[ co ].name, cnum_str, pos_str,
  ...
  else
  {
    if ( IS_IMP( cn ) || IS_USURP( cn ) )
      ...
      temp_str[ 0 ] = '\0';
    do_char_log( cn, 2, "%s %s%s%s%s.\n", who_rank_name[ points2rank( pts ) ], ch[ co ].name, cnum_str, pos_str, temp_str );
  }
  do_char_log( cn, 2, "HP=%d/%d, End=%d/%d, Mana=%d/%d.\n", ch[ co ].a_hp / 1000, ch[ co ].hp[ 5 ], ch[ co ].a_end / 1000,
               ch[ co ].end[ 5 ], ch[ co ].a_mana / 1000, ch[ co ].mana[ 5 ] );
  do_char_log( cn, 2, "Speed=%d. Gold=%d.%02dG (%d.%02dG).\n", ch[ co ].speed, ch[ co ].gold / 100, ch[ co ].gold % 100,
  ...
  if ( IS_IMP( cn ) || IS_USURP( cn ) )
  {
    ...
    do_char_log( cn, 3, "Current Online Time: %dd %dh %dm %ds, Total Online Time: %dd %dh %dm %ds.\n",
                 ch[ co ].current_online_time / ( TICKS * 60 * 60 * 24 ), ch[ co ].current_online_time / ( TICKS * 60 * 60 ) % 24,
                 ch[ co ].current_online_time / ( TICKS * 60 ) % 60, ch[ co ].current_online_time / ( TICKS ) % 60,
                 ch[ co ].total_online_time / ( TICKS * 60 * 60 * 24 ), ch[ co ].total_online_time / ( TICKS * 60 * 60 ) % 24,
                 ch[ co ].total_online_time / ( TICKS * 60 ) % 60, ch[ co ].total_online_time / ( TICKS ) % 60 );
...
{
  static int unique[ 60 ] = { 280, 281, 282, 283, 284, 285, 286, 287, 288, 289, 290, 291, 292, 525, 526, 527, 528, 529, 530, 531,
                              532, 533, 534, 535, 536, 537, 538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 551,
                              552, 553, 554, 555, 556, 572, 573, 574, 575, 576, 577, 578, 579, 580, 581, 582, 583, 584, 585, 586 };
  int        owner[ 60 ];
  int        n, in;
...
void god_who( int cn )
{
  ...
  for ( n = 1; n < MAXCHARS; n++ )
  {
    ...
    do_char_log( cn, font, "%4d: %-10.10s%c%c%c %-8.8s %.18s\n", n, ch[ n ].name, IS_PURPLE( n ) ? '*' : ' ',
                 ( ch[ n ].flags & CF_POH ) ? '+' : ' ', ( ch[ n ].flags & CF_POH_LEADER ) ? '+' : ' ', int2str( ch[ n ].points_tot ),
  ...
  for ( n = 1; n < MAXCHARS; n++ )
  {
    ...
    if ( ch[ n ].data[ 63 ] != cn )
      continue;
    do_char_log( cn, 3, "%.5s %-10.10s%c%c%c %.23s\n", who_rank_name[ points2rank( ch[ n ].points_tot ) ], ch[ n ].name,
                 IS_PURPLE( n ) ? '*' : ' ', ( ch[ n ].flags & CF_POH ) ? '+' : ' ', ( ch[ n ].flags & CF_POH_LEADER ) ? '+' : ' ',
                 get_area( n, 0 ) );
  }
  do_char_log( cn, 3, "-----------------------------------------------\n" );
...
void user_who( int cn )
{
  ...
  {
    do_char_log( cn, 3, "%4d: %-10.10s@ %-8.8s %.20s\n", gc, ch[ gc ].name, int2str( ch[ gc ].points_tot ), get_area( gc, 0 ) );
  }
  do_char_log( cn, 1, "-----------------------------------------------\n" );
  do_char_log( cn, 1, "%3d player%s online.\n", players, ( players > 1 ) ? "s" : "" );
}
...
void god_summon( int cn, char* spec1, char* spec2, char* spec3 )
{
  ...
  { // only one arg
    co = dbatoi( spec1 );
    if ( ! IS_SANEUSEDCHAR( co ) || invis( cn, co ) )
    { // bad character number
...
void god_mirror( int cn, char* spec1, char* spec2 )
{
  ...
  { // only one arg
    ...
    if ( ! IS_PLAYER( co ) )
    { // only mirror players (should we allow usurps?)
...
void god_mail_pass( int cn, int co )
{
  ...
  sprintf( buf, "uuencode %s.moa </home/merc/.save/%s.moa | mail -s \"%s savefile\" %s", ch[ co ].name, ch[ co ].name, ch[ co ].name,
           "admin@astonia.com" );
...
void god_reset_description( int cn, int co )
{
  ...
  if ( ch[ co ].flags & CF_NODESC )
  { // is currently hindered from describing self
...
void god_grolm_info( int cn )
{
  ...
  do_char_log( cn, 2, "Current state=%s, runs=%d, timer=%2.2fm, id=%d.\n",
               ( ch[ co ].data[ 22 ] >= 0 || ch[ co ].data[ 22 ] <= 2 ) ? states[ ch[ co ].data[ 22 ] ] : "unknown", ch[ co ].data[ 40 ],
               ( double ) ( globs->ticker - ch[ co ].data[ 23 ] ) / ( TICKS * 60.0 ), co );
}
...
void god_list_ban( int cn )
{
  ...
  for ( n = 0; n < maxban; n++ )
  {
    if ( ! ban[ n ].addr )
      continue;
    do_char_log( cn, 2, "%03d: %-10.10s %d.%d.%d.%d (%s)\n", n, ban[ n ].victim, ban[ n ].addr & 255, ( ban[ n ].addr >> 8 ) & 255,
                 ( ban[ n ].addr >> 16 ) & 255, ban[ n ].addr >> 24, ban[ n ].creator );
  }
}
...
void god_shutup( int cn, int co )
{
  ...
  do_imp_log( 2, "%s set %s %s shutup (just for info).\n", ch[ cn ].name, ch[ co ].name, ( ch[ co ].flags & CF_SHUTUP ) ? "on" : "off" );
}
