...
static char* _rank[ 24 ] = {
    ...
    "Master Sergeant", "First Sergeant",      "Sergeant Major",    "Second Lieutenant", "First Lieutenant", "Captain",
    "Major",           "Lieutenant Colonel",  "Colonel",           "Brigadier General", "Major General",    "Lieutenant General",
    "General",         "Field Marshal",       "Knight of Astonia", "Baron of Astonia",  "Earl of Astonia",  "Warlord of Astonia" };
...
static void who( void )
{
  ...
  for ( n = 1; n < MAXCHARS && shown < 50; n++ )
  {
    ...
    printf( "<tr><td><a href=\"/cgi-bin/info.cgi?cn=%d\">%s</a> &nbsp; &nbsp; </td><td>%s &nbsp; &nbsp; </td><td>%s &nbsp; &nbsp; "
            "</td><td>%s</td></tr>\n",
...
static void xtop( int srank )
{
  ...
  printf( "<table cellpadding=0 cellspacing=0 border=0>\n" );
  printf( "<tr><td>Name &nbsp; &nbsp;</td><td>Race &nbsp; &nbsp;</td><td>Rank &nbsp; &nbsp;</td><td>Active &nbsp; &nbsp; "
          "</td><td>PPM</td></tr>\n" );
...
static void dtop( int srank )
{
  ...
  for ( m = 0; m < 25; m++ )
  {
    ...
    printf( "<tr><td><a href=\"/cgi-bin/info.cgi?cn=%d\">%s</a> &nbsp; &nbsp; </td><td>%s &nbsp; &nbsp; </td><td>%s &nbsp; &nbsp; "
            ...
            nr[ m ], ch[ nr[ m ] ].name, racename( ch[ nr[ m ] ].kindred ), rank( ch[ nr[ m ] ].points_tot ),
            ( ch[ nr[ m ] ].used == USE_ACTIVE && ! ( ch[ nr[ m ] ].flags & ( CF_INVISIBLE | CF_NOWHO ) ) ? "Yes" : "No" ),
            ch[ nr[ m ] ].data[ 14 ] + ch[ nr[ m ] ].data[ 44 ] );
  }
  printf( "</table><br>\n" );
}
...
static void info( int cn )
{
  ...
  proz1 = 100.0 / ( ( t - globs->transfer_reset_time ) * ( ALLOWED_GB / 30 / 24 / 60 / 60 ) ) *
          ( ( globs->recv + globs->send ) / ( 1024.0 * 1024 * 1024 ) );
  proz2 = globs->load_avg / 100.0;

  // printf("<tr><td>Bandwidth usage: </td><td> %.4fGB recv, %.4fGB
  // send</td></tr>",(globs->recv/(1024.0*1024.0*1024.0)),(globs->send/(1024.0*1024.0*1024.0)));
  printf( "<tr><td>Allowed traffic: </td><td> %.3fGB</td></tr>",
          ( double ) ( t - globs->transfer_reset_time ) * ( ALLOWED_GB / 30 / 24 / 60 / 60 ) );
  printf( "<tr><td>Actual traffic: </td><td> %.3fGB </td><td> (%.2f%%)</td></tr>", ( globs->recv + globs->send ) / ( 1024.0 * 1024 * 1024 ),
          proz1 );
  printf( "<tr><td colspan=3><hr></td></tr>" );
  ...
  printf( "<tr><td>Uptime: </td><td>%lldh %lldm %llds</td></tr>", ( globs->uptime / ( TICKS * 60 * 60 ) ),
          ( globs->uptime / ( TICKS * 60 ) % 60 ), ( globs->uptime / ( TICKS ) ) % 60 );
  // printf("<tr><td>Total online time: </td><td>%lldh %lldm
  // %llds</td></tr>",(globs->total_online_time/(TICKS*60*60)),(globs->total_online_time/(TICKS*60)%60),(globs->total_online_time/(TICKS))%60);
  printf( "<tr><td colspan=3><hr></td></tr>" );
  ...
  printf( "Today is the %d%s%s%s%s day of the Year %d. It is %d:%02d Astonian Standard Time.<br><br>", globs->mdday,
...
void effects( void )
{
  ...
  for ( n = 0; n < MAXEFFECT; n++ )
  {
    if ( fx[ n ].used != USE_ACTIVE )
      continue;
    printf( "%d: Type=%d, Dur=%d, Data=%d,%d,%d<br>\n", n, fx[ n ].type, fx[ n ].duration, fx[ n ].data[ 0 ], fx[ n ].data[ 1 ],
            fx[ n ].data[ 2 ] );
  }
}
...
int main( int argc, char* args[] )
{
  ...
  printf( "    <table width=\"100%%\">"
          "        <tr>"
          "            <td align=\"center\"><a href=\"http://www.astonia.com/\"><img src=\"/gfx/logo.gif\" width=\"100\" height=\"60\" "
          "border=\"0\"></a></td>"
          "            <td align=\"center\">"
          "                <a href=\"/\">Home</a>"
