...
int sub_door_driver( int cn, int in )
{
  if ( it[ in ].data[ 0 ] == 65500 )
    return 0;
  if ( it[ in ].data[ 0 ] == 65501 || it[ in ].data[ 0 ] == 65502 )
...
int use_door( int cn, int in )
{
  ...
  if ( it[ in ].data[ 0 ] )
  {
    ...
    if ( ! lock && ( in2 = ch[ cn ].citem ) != 0 && ! ( in2 & 0x80000000 ) && it[ in2 ].driver == 3 )
...
int use_create_item( int cn, int in )
{
  ...
  if ( it[ in ].data[ 1 ] && it[ in ].driver == 53 )
  {
    ...
    sprintf( buf, "%s Engraved in it are the letters \"%s\".", it[ in2 ].description, ch[ cn ].name );
    if ( strlen( buf ) < 200 )
...
int teleport2( int cn, int in2 )
{
  ...
  chlog( cn, "Used teleport scroll to %d,%d (%s)", it[ in2 ].data[ 0 ], it[ in2 ].data[ 1 ],
         get_area_m( it[ in2 ].data[ 0 ], it[ in2 ].data[ 1 ], 0 ) );

  if ( it[ in2 ].data[ 2 ] && it[ in2 ].data[ 2 ] + TICKS * 60 * 4 < globs->ticker )
  {
    chlog( cn, "Lag Scroll Time Difference: %d ticks (%.2fs)", globs->ticker - it[ in2 ].data[ 2 ],
...
int use_pile( int cn, int in )
{
  ...
  static int find[] = { 361, 361, 339, 342, 345, 339, 342, 345, 359, 359, // silver, small jewels, skeleton
  ...
  { // something there?
    ...
    { // no? then it's an object which creates a monster
...
int use_pentagram( int cn, int in )
{
  ...
  ch[ cn ].data[ 18 ] = exp;
  do_char_log( cn, 1,
               "Your pentagrammas are worth a total of %d experience points. Note that only the highest 5 pentagrammas count towards your "
               "experience bonus.\n",
               exp );
...
int use_shrine( int cn, int in )
{
  ...
  if ( IS_SANEITEM( in2 ) &&
       ( strcmp( it[ in2 ].description, "ONE FIRE POINT" ) == 0 || strcmp( it[ in2 ].description, "ONE FAKE POINT" ) == 0 ) )
  {
...
int use_seyan_shrine( int cn, int in )
{
  ...
  sprintf( it[ in2 ].description, "A huge, two-handed sword, engraved with runes and magic symbols. It bears the name %s.", ch[ cn ].name );
...
int use_lab8_shrine( int cn, int in )
{
  ...
  if ( ( offer & 0x80000000 ) || ( it[ offer ].temp != it[ in ].data[ 0 ] ) )
  {
...
int shrine_of_change( int cn, int in )
{
  ...
  if ( it[ in2 ].temp == 127 || it[ in2 ].temp == 274 )
  ...
  if ( it[ in2 ].temp == 131 || it[ in2 ].temp == 273 )
  { // greater mana potion
    if ( ch[ cn ].kindred & KIN_MERCENARY )
...
int item_age( int in )
{
  ...
  if ( ! it[ in ].max_age[ act ] &&
       ( it[ in ].current_age[ act ] > TICKS * 60 * 30 || ( it[ in ].temp == 500 && it[ in ].current_age[ act ] > TICKS * 60 * 2 ) ) )
  {
