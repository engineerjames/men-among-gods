...
int npc_add_enemy( int cn, int co, int always )
{
  ...
  if ( ! ch[ cn ].attack_cn || ( d1 > d2 && ( globs->flags & GF_CLOSEENEMY ) ) ||
...
int npc_gotattack( int cn, int co, int dam )
{
  ...
  if ( co && ( ch[ co ].flags & CF_PLAYER ) && ch[ cn ].alignment == 10000 &&
       ( strcmp( ch[ cn ].name, "Peacekeeper" ) || ch[ cn ].a_hp < ch[ cn ].hp[ 5 ] * 500 ) && ch[ cn ].data[ 70 ] < globs->ticker )
  {
  ...
  if ( ch[ cn ].data[ 52 ] && ch[ cn ].a_hp < ch[ cn ].hp[ 5 ] * 666 )
  { // we're supposed to shout for help if attacked and HP below 2/3rd
    if ( ch[ cn ].data[ 55 ] + TICKS * 60 < globs->ticker )
    { // shout every 60 seconds
      ch[ cn ].data[ 54 ] = 0;
  ...
  { // we have been attacked but cannot see the attacker
...
int npc_seeattack( int cn, int cc, int co )
{
  ...
  if ( ch[ co ].temp == CT_COMPANION && ch[ co ].data[ 63 ] == cn )
  { // MY ghost companion
    if ( ! ch[ cn ].data[ 65 ] )
    {
      ch[ cn ].data[ 65 ] = co;
  ...
  if ( ch[ cc ].temp == CT_COMPANION && ch[ cc ].data[ 63 ] == cn )
  { // MY ghost companion
    if ( ! ch[ cn ].data[ 65 ] )
    {
      ch[ cn ].data[ 65 ] = cc;
...
int npc_give( int cn, int co, int in, int money )
{
  ...
  {
    if ( ch[ cn ].data[ 49 ] == 740 && ch[ cn ].temp == 518 )
...
int npc_cityguard_see( int cn, int co, int flag )
{
  ...
  if ( ch[ co ].data[ 42 ] == 27 )
  { // shout if enemy in sight (!)
    if ( ch[ cn ].data[ 55 ] + TICKS * 180 < globs->ticker )
...
int is_scroll( int in )
{
  ...
  tn = it[ in ].temp;
  return ( ( ( tn >= 699 ) && ( tn <= 716 ) ) || ( ( tn >= 175 ) && ( tn <= 178 ) ) || ( ( tn >= 181 ) && ( tn <= 189 ) ) );
}
...
int npc_see( int cn, int co )
{
  ...
  if ( ! ch[ cn ].attack_cn )
  { // only attack him if we aren't fighting already
    idx = co | ( char_id( co ) << 16 );
  ...
  if ( ! ch[ cn ].attack_cn && ( ch[ co ].flags & ( CF_PLAYER ) ) && ch[ cn ].data[ 37 ] && indoor1 == indoor2 &&
       ch[ cn ].data[ 56 ] < globs->ticker )
  {
    for ( n = 37; n < 41; n++ )
      if ( ch[ cn ].data[ n ] == co )
        break;
    if ( n == 41 )
    {
      ...
      {
        if ( ( ch[ co ].kindred & KIN_TEMPLAR ) || ( ch[ co ].kindred & KIN_ARCHTEMPLAR ) ||
      ...
      else
      {
        ...
        { // priest template is 180
          do_sayx( cn, "Greetings, %s!", ch[ co ].name );
        }
        else
        { // if it is not priest or player is not purple we do nothing, NPC will talk as usual
...
int npc_hitme( int cn, int co )
{
  ...
  if ( ch[ cn ].data[ 26 ] == 4 || ch[ cn ].data[ 26 ] == 2 )
  { // generic or shiva
    if ( npc_add_enemy( cn, co, 1 ) )
    {
      npc_saytext_n( cn, 1, ch[ co ].name );
...
int npc_try_spell( int cn, int co, int spell )
{
  ...
  for ( n = 0; n < 20; n++ )
  {
    ...
    {
      if ( it[ in ].temp == spell &&
           it[ in ].power + 10 >= spell_immunity( ch[ cn ].skill[ spell ][ 5 ], ch[ co ].skill[ SK_IMMUN ][ 5 ] ) &&
           it[ in ].active > it[ in ].duration / 2 )
        break;
    }
  }
...
int npc_driver_high( int cn )
{
  ...
  if ( ch[ cn ].temp == CT_COMPANION && ch[ cn ].data[ 64 ] == 0 )
  {
    ...
    if ( ! IS_SANECHAR( co ) || ch[ co ].data[ 64 ] != cn )
    { // mismatch
  ...
  if ( ( co = ch[ cn ].attack_cn ) != 0 || ch[ cn ].data[ 78 ] )
  { // we're fighting
    ...
    if ( co && ( ch[ cn ].a_hp < ch[ cn ].hp[ 5 ] * 600 || ! RANDOM( 10 ) ) )
    { // we're losing
    ...
    if ( co && ch[ co ].armor + 5 > ch[ cn ].weapon )
    ...
    for ( x = std::max( ch[ cn ].x - 8, 1 ); x < std::min( ch[ cn ].x + 8, MAPX - 1 ); x++ )
    {
      if ( ( in = map[ x + y * MAPX ].it ) != 0 )
      {
        ...
        if ( ch[ cn ].data[ 47 ] && indoor1 == indoor2 && ( it[ in ].flags & IF_TAKE ) &&
             can_go( ch[ cn ].x, ch[ cn ].y, it[ in ].x, it[ in ].y ) && do_char_can_see_item( cn, in ) && it[ in ].temp != 18 )
        {
...
int npc_loot_grave( int cn, int in )
{
  ...
  if ( abs( ch[ cn ].x - it[ in ].x ) + abs( ch[ cn ].y - it[ in ].y ) > 1 ||
       drv_dcoor2dir( it[ in ].x - ch[ cn ].x, it[ in ].y - ch[ cn ].y ) != ch[ cn ].dir )
  {
    ch[ cn ].misc_action  = DR_USE;
...
void npc_driver_low( int cn )
{
  ...
  if ( ch[ cn ].data[ 25 ] )
  { // check for special driver routine
    switch ( ch[ cn ].data[ 25 ] )
    {
    case 1:
      npc_stunrun_low( cn );
  ...
  if ( ch[ cn ].alignment < 0 && ( globs->flags & GF_LOOTING ) &&
       ( ( cn & 15 ) == ( globs->ticker & 15 ) || ( ch[ cn ].flags & CF_ISLOOTING ) ) && ch[ cn ].temp != CT_COMPANION )
  {
