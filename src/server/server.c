...
void chlog( int cn, char *format, ... )
{
  ...
  fprintf( logfp, "%02d.%02d.%02d %02d:%02d:%02d: %s %s%s%s(%d at %d,%d from %d.%d.%d.%d, ID=%lld): %s\n", tm->tm_mday, tm->tm_mon + 1,
           tm->tm_year - 100, tm->tm_hour, tm->tm_min, tm->tm_sec, name, usurp ? "[" : "", usurp ? usurp : "", usurp ? "] " : "", cn,
           ch[ cn ].x, ch[ cn ].y, addr & 255, ( addr >> 8 ) & 255, ( addr >> 16 ) & 255, addr >> 24, unique, buf );
  fflush( logfp );
}
...
void plog( int nr, char *format, ... )
{
  ...
  fprintf( logfp, "%02d.%02d.%02d %02d:%02d:%02d: %s (%d at %d,%d from %d.%d.%d.%d, ID=%lld): %s\n", tm->tm_mday, tm->tm_mon + 1,
           tm->tm_year - 100, tm->tm_hour, tm->tm_min, tm->tm_sec, name, cn, x, y, addr & 255, ( addr >> 8 ) & 255, ( addr >> 16 ) & 255,
           addr >> 24, unique, buf );
  fflush( logfp );
}
